...
class FileUpload
{
    ...
    public function uploadSingleFile($name)
    {
        ...
        if($typeOK && $sizeOK)
        {
            $this -> location = $this -> location . mt_rand(0000, 9999) . date('jnYGis') . $_FILES[$name]['name'];
        ...
        else
        {
            ...
            throw new Exception('Your size exceeds ' . $this -> originalSize . strtoupper($this -> unit) . '. The accepted file types are: ' . $validType);
        }
        return $this -> result;
    }
            ...
            case true:
                ...
                if(empty($failedSizeTest))
                {
                    ...
                    if(empty($failedTypeTest))
                    {
                        ...
                        {
                            throw new Exception('The following images did not upload due to system error: ' . join(', ', $failedTypeTest) . '.');
                        }
                    }
                    else
                    {
                        throw new Exception('The following images did not upload because of their file types: ' . join(', ', $failedTypeTest) . '. The accepted file types are: ' . join(', ', $this -> formatExt()));
                    }

                }
                else
                {
                    throw new Exception('The following images exceeds ' . $this -> originalSize . strtoupper($this -> unit) . ': <br />' . join(',<br />', $failedSizeTest));
                }
                break;
            case false:
